layui.define([], function (exports) {
	...
	let tab = {
		...
		*/
		...
		tabCookie: function() {
			let thetabs = $('#pageTabUl').find('li'), tab_id = 0, tab_str = '';
			thetabs.each(function(index, item) {
				let _id = $(item).attr('lay-id'),
					_url = $(item).attr('lay-url'),
					_title = $(item).text();
				if (_id > 0) {
					let tab_item = 'id:' + _id + ',url:' + _url + ',title:' + _title;
					if (tab_str) {
						tab_str += '|';
					}
					tab_str += tab_item;
			...
			if (tab_str) {
				let tabs_str = 'tab_id:' + tab_id + ',tab_array[' + tab_str + ']';
				tab.setCookie('gougutab', tabs_str);
			} else {
		...
		getCookie: function (name) {
			...
			if (arr != null) {
				let cookieValue = decodeURIComponent(arr[2]);  // 解码 cookie 内容
				let tab_id = 0, tab_array = [];
				let tab_id_match = cookieValue.match(/tab_id:(\d+)/);
				let tab_array_match = cookieValue.match(/tab_array\[([^\]]+)\]/);
				if (tab_id_match) {
					tab_id = parseInt(tab_id_match[1]);
				}
				if (tab_array_match) {
					let tab_items = tab_array_match[1].split('|');
					tab_array = tab_items.map(item => {
						let tab = {};
